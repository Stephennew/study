<?php 
	...
	class MysqlDb{
		...
		public function query($sql){
			...
			if($result === false){
				die('错误sql:'.$sql.'错误代码:'.mysqli_errno($this->link).'错误信息:'.msyqli_error($this->link));
			}
			return $result;
		}
		/*公有的静态的创建对象的方法*/
		public static function getIstance($arr){
			if(!self::$object instanceof static){//self::$object == null
				self::$object = new static($arr);	
			}
			return self::$object;
		}
		/*关联数组,返回多条数据*/
