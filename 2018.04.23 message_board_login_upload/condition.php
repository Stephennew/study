...
/*$write_list=['list','report'];
//需要登录后才可以使用的功能
if(!in_array($act, $write_list)){
	if(!isset($_SESSION['userinfo'])){
		echo '您没有权限,请先登录!';
		header('Refresh: 1; url=condition.php?act=login');
		die();
	}
...
if($act == 'login'){
	...
	if($row['username'] != $username){
		echo '该用户不存在,请重新登录';
		header('Refresh: 1; url=condition.php?act=login'); //延迟转向 也就是隔几秒跳转
		die;
	}
	if($row['pwd'] != $pwd){
	...
	$total_num=mysqli_fetch_row($result2)[0]; //结果是一个数组,可以取值
	...
	/*array(1) {
	  ...
	  array(5) {
	    ...
	    string(9) "image/gif"
	    ["tmp_name"]=>
	    string(27) "C:\Windows\Temp\phpC19F.tmp"
	    ["error"]=>
	    int(0)
	    ["size"]=>
	    int(13580)
	...
	if($_FILES['icon']['error'] == 0){
		...
		if(in_array($_FILES['icon']['type'], $type)){
			...
			$new_name = './upload_file/'.uniqid().mt_rand(1000,9999999).$suffix;

			move_uploaded_file($tmp_name, $new_name);

		}else{
			echo '您上传的图片格式不正确,请重新上传!';
			die;
		}
	}
	//1.连接数据库
	...
	$sql ="insert into message set name='{$name}',date='{$add_date}',content='{$content}',icon='{$new_name}'";
	//var_dump($sql);die();
	//3.执行sql
	$result=mysqli_query($link,$sql);
	//var_dump($result);die();
	...
	//echo '留言成功';
	header('Location: condition.php?act=list'); //跳转到一个新的地址 
	//header('Refresh: 1; condition.php?act=list'); //延迟转向 也就是隔几秒跳转
